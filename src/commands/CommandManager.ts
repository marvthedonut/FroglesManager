...
export default class CommandManager {
	...
	public static registerCommands = async (client: Client) => {
		fs.readdirSync(this.commandsDir).filter((f: string) => f.endsWith(".js") && !this.commandsIgnored.includes(f)).forEach((commandName: string) => this.registerCommand(commandName));
	}
